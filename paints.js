...
class FloodFillGame {
    ...
    async loadTranslations() {
        try {
            const response = await fetch(`js/langs/paints-${this.currentLanguage}.json`);
            this.translations = await response.json();
        } catch (error) {
            console.error('Failed to load translations:', error);
            // 加载默认中文翻译
            const response = await fetch('js/langs/paints-zh.json');
            this.translations = await response.json();
        }
    }
    ...
    updateAllTexts() {
        ...
        document.getElementById('gameTitle').textContent = this.translations.title;
        document.getElementById('movesLeftLabel').textContent = this.translations.ui.movesLeft;
        document.getElementById('targetColorLabel').textContent = this.translations.ui.targetColor;
        document.getElementById('gridSizeLabel').textContent = this.translations.ui.gridSize;
        document.getElementById('colorCountLabel').textContent = this.translations.ui.colorCount;
        document.getElementById('instructionText').textContent = this.translations.ui.instruction;
        
        document.getElementById('reduceDifficultyBtn').textContent = this.translations.buttons.reduceDifficulty;
        document.getElementById('retryBtn').textContent = this.translations.buttons.retry;
        document.getElementById('newGameBtn').textContent = this.translations.buttons.newGame;

        // 更新颜色名称
        this.colorNames = this.translations.ui.colorNames;
        this.updateColorTooltips();
    }
    ...
    updateColorTooltips() {
        ...
        colorSamples.forEach((sample, index) => {
            if (this.colorNames[index]) {
                sample.title = this.colorNames[index];
            }
        });
    }
    ...
    showSuccessMessage() {
        const message = this.translations.messages?.success || '恭喜！你成功完成了溢彩画！🎉';
        this.showGameStatus(message, 'status-success');
    }

    showFailureMessage() {
        const message = this.translations.messages?.failure || '游戏结束！步数用完了，再试试看吧！😔';
...
async function toggleLanguage() {
    game.currentLanguage = game.currentLanguage === 'zh' ? 'en' : 'zh';
    await game.loadTranslations();
    game.updateLanguageDisplay();
}
